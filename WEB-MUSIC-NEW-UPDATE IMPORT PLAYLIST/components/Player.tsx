...
export function Player() {
  ...
  const hasEndedRef = useRef(false); // prevent double-trigger on ENDED
  ...
  useEffect(() => {
    if (
      ...
      const index = lyrics.findIndex(
        (line) =>
          line.time !== undefined && line.time > progress + LYRICS_OFFSET,
      );
      const activeIndex =
  ...
  const onStateChange = useCallback(
    async (event: any) => {
      if (event.data === YouTube.PlayerState.PLAYING) {
        ...
        if (usePlayerStore.getState().isPlaying) {
          // Browser paused it automatically (e.g., went to background)
          // Force play again to maintain background playback
    ...
    [setPlaying, setDuration, handleTrackEnd],
  );
      ...
      if (
        ...
        try {
          ...
          const res = await fetch(
            `/api/search?q=${encodeURIComponent(query)}&type=video`,
          );
          if (res.ok) {
            const videos = await res.json();
            const alternativeVideo = videos.find(
              (v: any) => v.videoId && v.videoId !== currentTrack.videoId,
            );
            if (alternativeVideo) {
  ...
  // Progress interval — 1000ms untuk hemat baterai, dengan fallback deteksi lagu habis
  useEffect(() => {
    ...
    if (isPlaying) {
      interval = setInterval(async () => {
        if (
          ...
          // Fallback: deteksi lagu habis via interval (untuk background/Kiwi browser)
          if (
            duration > 0 &&
            time > 0 &&
            time >= duration - 1.5 &&
      ...
      }, 1000); // 1 detik, hemat baterai vs 50ms sebelumnya
    }
    return () => clearInterval(interval);
  }, [isPlaying, setProgress, duration, handleTrackEnd]);

  // Media Session API
  useEffect(() => {
    if (currentTrack && "mediaSession" in navigator) {
      const thumbnail = getHighResImage(
  ...
  // AudioContext unlock for background audio
        ...
        {!isExpanded && (
          ...
          >
                    ...
                    text={
                      <>
                        {currentTrack.isExplicit && (
                          <span className="bg-white/20 text-[8px] px-1 rounded-sm text-white">
                            E
                          </span>
                        )}
                        {artistName}
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              ...
              >
                <Heart
                  className={`w-5 h-5 ${isLiked ? "fill-[#FA243C] text-[#FA243C]" : ""}`}
                />
              </button>
            </div>
          </motion.div>
                    ...
                    >
                      {lyrics ? (
                                ...
                                className={cn(
                                  ...
                                  lyricsType === "synced"
                                    ? isActive
                                      ...
                                      : "text-white/30 scale-100 cursor-pointer hover:text-white/60"
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 h-full">
                          ...
                          <span className="text-white/50 text-xl font-medium tracking-wide">
                            Memuat lirik...
                          </span>
                        </div>
                      )}
            ...
            <div className="mt-8">
              ...
              <div className="flex justify-between items-center px-6 py-4 bg-white/5 rounded-2xl">
                ...
                <button
                  ...
                  className={cn(
                    "transition flex flex-col items-center gap-1",
                    showLyrics
                      ? "text-white"
                      : "text-white/80 hover:text-white",
                  )}
